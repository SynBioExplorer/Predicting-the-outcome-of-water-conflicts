...
Transboundary water interactions shape geopolitical stability for billions of people, yet quantitative prediction of conflict outcomes remains underdeveloped. Here we present, to our knowledge, the first ordinal-aware machine learning benchmark on the Transboundary Freshwater Dispute Database (TFDD), analysing 6,805 water interaction events across 313 international river basins from 1948 to 2008. Using a systematic ablation protocol with robustness testing across multiple feature group orderings, we evaluate eight external data sources and find that economic indicators and temporal dynamics are candidate predictors of conflict intensity, though their incremental value is order-dependent. Basin-averaged annual climate indicators show ambiguous incremental value that depends on ablation ordering, though grouped permutation importance suggests a collective positive contribution. Our primary model, an XGBoost classifier tuned via nested cross-validation on 42 non-autoregressive features, achieves a test-set quadratic weighted kappa (QWK) of 0.298 and macro-F1 of 0.320, outperforming ordinal regression baselines. Including autoregressive features (cooperation momentum, prior event counts) inflates validation QWK to 0.502 but does not improve test generalization, confirming endogeneity risk. SHAP temporal decomposition reveals that treaty formation rate increased 56.5% in importance from the Cold War to the post-2000 era, supporting the institutional-change hypothesis. Continent-level spatial cross-validation yields a mean QWK of 0.248 with substantial geographic variation (0.062 to 0.417), indicating that event-level water conflict prediction remains a fundamentally difficult task requiring region-specific calibration.
...
We structure the discussion around four policy-relevant insights that emerge from the analysis, followed by methodological implications. An important epistemological caveat applies throughout: SHAP values measure marginal feature contributions to model predictions, not causal effects on real-world outcomes (Lundberg & Lee, 2017). Features with high SHAP importance may be proxies for true causal factors, reflect confounding, or capture spurious correlations. Without causal identification strategies (directed acyclic graphs, instrumental variables, or natural experiments), all interpretive claims below are associational hypotheses warranting further investigation through causal research designs, not established causal relationships. We use language such as "consistent with" and "suggests" throughout to reflect this distinction.

### Institutional velocity matters more than institutional stock
...
### Economic capacity as a governance prerequisite
...
### Climate as a structural mediator, not a proximate trigger
...
### Geopolitical regime dependence limits operational forecasting

The validation-to-test performance gap carries a direct policy implication: early warning systems for transboundary water conflict cannot be trained once and deployed indefinitely. The class distribution shifts substantially across temporal regimes (conflict class rises from 16.3% in training to 24.8% in test; strong cooperation drops from 30.0% to 9.7%), and rolling-window analysis (QWK 0.145 to 0.280 across windows) confirms that predictive accuracy varies substantially by historical period. The validation period (1996-2002) may represent an unusually predictable epoch of post-Cold War institutional optimism, while the post-2003 test period reflects the destabilizing effects of the Iraq War on Middle Eastern water diplomacy and broader geopolitical realignment.

For practitioners building conflict early warning systems, this finding implies that operational tools must incorporate continuous model retraining as geopolitical conditions evolve, require region-specific calibration (spatial CV shows QWK ranging from 0.062 in North America to 0.417 in Europe, Table 5), and provide probabilistic outputs with explicitly communicated uncertainty rather than point predictions. The WPS Partnership's operational tool exemplifies this approach, explicitly stating that it "is not intended to elucidate causal relationships between the predictor variables and conflict," an epistemic humility that our analysis reinforces. Our ordinal framework complements binary early warning tools by providing intensity gradations rather than binary conflict/no-conflict predictions, but shares the fundamental limitation that historical patterns are unreliable guides to novel geopolitical configurations.

### Governance data gaps as a policy blind spot
...
This study applies ordinal machine learning to the TFDD to investigate the conditions under which transboundary water interactions escalate or cooperate, yielding four policy-relevant insights and two methodological contributions.

The policy insights, presented as associational hypotheses requiring causal confirmation, are: (1) the pace of institutional adaptation (treaty formation rate) is a stronger correlate of cooperative outcomes than the stock of existing treaties, particularly in the post-Cold War era, supporting Wolf et al.'s (2003) institutional-change hypothesis and suggesting that investment in institutional velocity deserves priority alongside permanent treaty instruments; (2) economic capacity functions as a consistent predictor of conflict intensity across model specifications, consistent with the hydro-hegemony framework and implying that development assistance and capacity-building serve dual functions of addressing water scarcity and strengthening governance foundations; (3) annual basin-averaged climate indicators at current resolution show ambiguous incremental predictive value, consistent with climate operating as a diffuse structural mediator rather than a proximate trigger, though grouped permutation importance (+0.019 QWK) suggests collective climate signal exists that sequential ablation fails to capture; and (4) governance data gaps in conflict-prone regions constitute a policy blind spot, with 60-64% missingness in institutional quality indicators likely causing governance features to be erroneously discarded, and the informativeness of missingness patterns suggesting that data infrastructure investment is itself a conflict prevention tool.

The methodological contributions are: (1) the demonstration that autoregressive features (cooperation momentum, prior event counts) inflate validation metrics by nearly 0.10 QWK without improving test generalization (test QWK 0.298 without vs 0.290 with autoregressive features), providing a concrete warning against endogenous feature construction in conflict prediction; and (2) the finding that sequential feature ablation is path-dependent (different orderings retain different feature groups from the same candidate pool), cautioning against interpreting ablation rankings as definitive.

For the water governance community, the central message is that conflict prediction models trained on historical data do not transfer reliably across geopolitical regimes (validation-to-test gap of 26%) or geographic contexts (spatial CV QWK ranging from 0.062 to 0.417). Operational early warning systems require continuous retraining, region-specific calibration, and explicitly communicated uncertainty. Future work should extend this framework to the post-2008 period using updated TFDD releases, re-run the ablation protocol with native missing-value handling to reassess governance features, test sub-basin and sub-annual climate indicators that may capture proximate hydrological triggers, and develop causal identification strategies to distinguish the associational patterns identified here from genuine institutional mechanisms.
