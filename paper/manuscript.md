...
The methodological caution is equally important: the validation-to-test QWK degradation (26% for the primary 42-feature model, rising to 42% when autoregressive features inflate validation metrics), the path-dependence of ablation decisions, and the substantial geographic variation in spatial cross-validation (QWK 0.062 to 0.417 across continents) collectively indicate that event-level water conflict prediction at this resolution remains a fundamentally difficult task. Models trained on historical data do not transfer reliably across geopolitical regimes or geographic contexts without retraining and region-specific calibration. Future work should extend this framework to the post-2008 period using updated TFDD releases, re-run the ablation protocol with native NaN handling to reassess governance features, test sub-basin and sub-annual climate indicators (particularly minimum monthly SPEI) that may capture proximate hydrological triggers, and explore non-endogenous approaches to temporal dynamics.
